...
uint256 hashGenesisBlock("0xf1abc6541f3e268538fc991d81ffd6f5937d7c70bd30e26cb170ad8ea249ef3a");
...
const string strMessageMagic = "BitzRichCoin Signed Message:\n";
...
{
    // BitzRichCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BitzRichCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "brn genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1521842001;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1088977;
...
unsigned char pchMessageStart[4] = { 0xda, 0xcf, 0xae, 0xe3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitzRichCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitzRichCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitzRichCoinMiner(CWallet *pwallet)
{
    printf("BitzRichCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitzRichCoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitzRichCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitzRichCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitzRichCoinMiner, pwallet));
}
