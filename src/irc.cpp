...
// Copyright (c) 2011-2012 BitzRichCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("BitzRichCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitzRichCoinTEST0\r");
            Send(hSocket, "WHO #BitzRichCoinTEST0\r");
        } else {
            // randomly join #BitzRichCoin00-#BitzRichCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #BitzRichCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BitzRichCoin%02d\r", channel_number).c_str());
        }
